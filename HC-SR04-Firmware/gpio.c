...
//since every register has 32 bits then, first reg = 4 bytes, second reg starts at 32th bit. hence offset*4*8
...
void turnOnLED(PORT port,uint8_t pin, bool onOff)
{
    enablePort(port);
    enablePinPullup(port,pin);
    selectPinPushPullOutput(port,pin);
    setPinValue(port,pin, onOff);
}
